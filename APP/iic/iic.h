...
void IICB_Init(void);                //初始化IIC的IO口
void IIC_B_Start(void);               //发送IIC开始信号
void IIC_B_Stop(void);                //发送IIC停止信号
void IIC_B_Send_Byte(unsigned char txd);          //IIC发送一个字节
unsigned char IIC_B_Read_Byte(unsigned char ack);//IIC读取一个字节
unsigned char IIC_B_Wait_Ack(void);               //IIC等待ACK信号
